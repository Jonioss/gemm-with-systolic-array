...
void gemm(const hls::burst_maxi<hls::vector<float, VEC_SIZE>> A_DRAM, 
	...
	#pragma HLS INTERFACE m_axi offset=slave port=A_DRAM bundle=gmem0 depth=I*K/VEC_SIZE max_read_burst_length=BURST_LENGTH latency=64
	#pragma HLS INTERFACE m_axi offset=slave port=B_DRAM bundle=gmem1 depth=K*J/VEC_SIZE max_read_burst_length=BURST_LENGTH latency=64
	#pragma HLS INTERFACE m_axi offset=slave port=C_DRAM bundle=gmem2 depth=I*J/VEC_SIZE max_write_burst_length=BURST_LENGTH latency=64
